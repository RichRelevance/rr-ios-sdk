...
+ (NSArray *)sectionsForObjects:(NSArray *)objects withNameKeyPath:(NSString *)keyPath
{
    ...
    if (nil == keyPath) {
        ...
        [objects enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if (nil == sectionName) {
                @throw [NSException exceptionWithName:@"RZArrayCollectionListInvalidSecionNameKeyPath" reason:[NSString stringWithFormat:@"An object at index %lu returned nil for the keyPath:%@", (unsigned long)idx, keyPath] userInfo:@{ @"keyPath" : keyPath,
